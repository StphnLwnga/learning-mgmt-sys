...
const ChapterDescriptionForm = ({ initialData, courseId }: ChapterDescriptionFormProps): JSX.Element => {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>): Promise<void> => {
    try {
      await axios.patch(`/api/courses/${courseId}/chapters/${initialData.id}`, data);
      toast({
        title: 'Success',
        ...
        >
          {isEditing
            ...
            : !initialData.description
              ? (<>Add chapter description <PlusCircle className='h-4 w-4 ml-2' /></>)
              : (<>Edit chapter description <Pencil className='h-4 w-4 ml-2' /></>)
          }
        </Button>
      </div>
      {!isEditing && (
